...
async def test_heating_thermostat_three(no_ssl_fake_async_home: AsyncHome):
    ...
    assert str(d) == (
        "HmIP-eTRV-3 Heizkoerper lowBat(False) unreach(False) "
        "rssiDeviceValue(-50) rssiPeerValue(-49) configPending(False) "
        "dutyCycle(False) mountingOrientation(RIGHT) operationLockActive(False) valvePosition(0.0) "
        "valveState(ADAPTION_DONE) temperatureOffset(0.0) "
