...
- Fix `set_pin_async` leaking the old PIN into the connection's persistent headers (subsequent requests carried it indefinitely)
- Fix SSL verification not actually disabled when `enforce_ssl=False` (REST + websocket)
...
- Add ruff linter with CI enforcement (pyflakes, isort, pyupgrade, bugbear)

### Fixed

- Fix ELV-SH-SPS25 missing switch entity in HA (map to PlugableSwitch instead of base Switch)
...
- Add support for HmIP-WGT (thanks to @lackas !)

## [2.4.0](https://github.com/hahn-th/homematicip-rest-api/compare/2.3.1..2.4.0) 2025-11-10

### Added

- Add support for HMIP-FALMOT-C8 (#605)
